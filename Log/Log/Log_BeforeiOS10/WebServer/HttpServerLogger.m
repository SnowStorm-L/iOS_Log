...
- (void)startServer{
    [self.webServer startWithPort:kWebServerPort bonjourName:nil];
}
...
- (void)_appendLogRecordsToString:(NSMutableString*)string afterAbsoluteTime:(double)time {
    ...
    NSArray<SystemLogMessage *>  *allMsg = [SystemLog allLogAfterTime:time];
...
- (NSString *)displayedTextForLogMessage:(SystemLogMessage *)msg {
    ...
    [string appendFormat:@"<td></td> <td></td> <td>%@</td>", msg.messageText];
    return string;
}
